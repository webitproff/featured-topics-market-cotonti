...
<div class="alert alert-success">
	<div class="customrelated-container">
		<h3 class="fs-5 my-2">{PHP.L.featuredtopicsmarket_edit_title}</h3>
		<!-- BEGIN: RECOMMENDED_FR_TOPIC_MARKET_ROW -->
		<div class="customrelated-row mb-3">
			<label class="form-label">
				{PHP.L.featuredtopicsmarket_item_number} {RECOMMENDED_FR_TOPIC_MARKET_NUM}
			</label>
			<div class="input-group">
			
				<!-- Это поле отправляется на сервер -->
				<input type="hidden"
					   name="recommended_topic_id[{RECOMMENDED_FR_TOPIC_MARKET_INDEX}]"
					   class="customrelated-id"
					   value="{RECOMMENDED_FR_TOPIC_MARKET_TO_ID}" />

				<!-- select ТОЛЬКО для интерфейса, name убираем полностью -->
				<select class="customrelated-select-topics "
						data-placeholder="{PHP.L.featuredtopicsmarket_selectpage_hint}">
					<!-- IF {RECOMMENDED_FR_TOPIC_MARKET_TO_ID} > 0 -->
					<option value="{RECOMMENDED_FR_TOPIC_MARKET_TO_ID}" selected>{RECOMMENDED_FR_TOPIC_MARKET_TO_TITLE}</option>
					<!-- ENDIF -->
				</select>
			</div>
			<small class="form-text text-muted">
				{PHP.L.featuredtopicsmarket_min_search}
			</small>
		</div>
		<hr>
...
<!-- END: MAIN -->
/**
 * featuredtopicsmarket.edit.topics.tpl - Template File for the Plugin Recommended Forum Topics in Market Item
 *
 * featuredtopicsmarket plugin for Cotonti 0.9.26, PHP 8.4+ 
 * Filename: featuredtopicsmarket.edit.topics.tpl
 *
 * Date: Jan 23Th, 2026 
